...
function renderDropdown(input) {
    ...
    filtered.slice(0, 12).forEach(h => {
        ...
        dropdown.appendChild(item); // Ошибка была тут, поправлено ниже:
        d.appendChild(item);
    });
...
function createCard(h, type, label) {
    ...
    div.innerHTML = `
        ...
        <div class="top-pick-hero-stats">
            Винрейт: <span class="val-green">${h.winrate}%</span><br>
...
function createHeroSmallCard(h) {
    ...
    const cl = h.advantage >= 0 ? 'var(--accent-green)' : 'var(--accent-red)';
    const sign = h.advantage >= 0 ? '+' : '';
    div.innerHTML = `<img src="${h.img}"><div class="hero-info"><div class="hero-name">${h.name} <span style="color:${cl}; font-size:0.75rem;">${sign}${h.advantage}%</span></div><div class="hero-stats">WR: ${h.winrate}%</div></div>`;
    return div;
}
