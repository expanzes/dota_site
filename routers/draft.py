...
HEROES_CACHE, HERO_STATS_CACHE = [], []
MATCHUP_CACHE, LANE_ROLES_CACHE = {}, {}
MIN_GAMES_FOR_RANK, MIN_GAMES_FOR_MATCHUP = 50, 30
OTHERS_COUNT, MIN_LANE_SHARE = 3, 0.10

ROLES = {"pos1": "Поз 1 (Керри)", "pos2": "Поз 2 (Мид)", "pos3": "Поз 3 (Тройка)", "pos4": "Поз 4 (Четверка)", "pos5": "Поз 5 (Пятерка)"}
LANE_FOR_POSITION = {1: {1}, 2: {2}, 3: {3}, 4: {1, 3}, 5: {1}}
...
async def get_hero_stats_cache():
    ...
    async with httpx.AsyncClient() as c:
        r = await c.get("https://api.opendota.com/api/heroStats")
        if r.status_code == 200: HERO_STATS_CACHE = r.json()
...
async def get_lane_roles_cache():
    ...
    async with httpx.AsyncClient() as c:
        r = await c.get("https://api.opendota.com/api/scenarios/laneRoles")
        if r.status_code == 200:
            agg = {}
            for row in r.json():
...
async def get_global_bans(user_id: str):
    ...
    matchups_data = await asyncio.gather(*(get_matchups(fid) for fid in fav_ids))
    ...
    for matchups in matchups_data:
        for m in matchups:
            ...
            name = id_to_name.get(m.get("hero_id"))
            if name: threats[name] = threats.get(name, 0) + (m.get("wins", 0) / games)
...
async def analyze_draft(payload: DraftRequest):
    ...
    base_wr = {hs["localized_name"]: (sum(hs.get(f"{i}_win", 0) for i in range(1, 9)) / sum(hs.get(f"{i}_pick", 0) for i in range(1, 9)) * 100) if sum(hs.get(f"{i}_pick", 0) for i in range(1, 9)) else 50 for hs in hero_stats if "localized_name" in hs}
    base_gm = {hs["localized_name"]: sum(hs.get(f"{i}_pick", 0) for i in range(1, 9)) for hs in hero_stats if "localized_name" in hs}
    ...
    hero_advs = {}
    for matchups in matchups_results:
        for m in matchups:
            name = id_to_name.get(m["hero_id"])
            if name and m["games_played"] >= MIN_GAMES_FOR_MATCHUP:
    ...
    for h in heroes_list:
        ...
        adv_list = hero_advantages = hero_advs.get(h["name"], [0])
    ...
    for r_k, r_t in ROLES.items():
        ...
        pos = int(r_k[-1])
        eligible = [c for c in all_candidates if pos in HERO_POSITIONS.get(c["name"], [1,2,3,4,5])]
