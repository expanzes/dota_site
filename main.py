...
# Используем твою переменную SESSION_SECRET из Render
app.add_middleware(
    SessionMiddleware, 
...
# --- УСКОРЕНИЕ: КЭШ СТРАНИЦ В ОПЕРАТИВНОЙ ПАМЯТИ ---
HTML_CACHE = {}

def serve_page(file_name: str, css_module: str = None):
    # Если страницы еще нет в памяти — читаем её с диска сервера
    if file_name not in HTML_CACHE:
        file_path = f"static/{file_name}"
        if not os.path.exists(file_path):
            return HTMLResponse(content="<h1>404</h1>", status_code=404)
        with open(file_path, "r", encoding="utf-8") as f:
            HTML_CACHE[file_name] = f.read()
    
    # Берем готовую строку из памяти
    html = HTML_CACHE[file_name]
    
    # Вклеиваем актуальные версии стилей
