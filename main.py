...
ROLES_DB = {
    "pos1": {
        "phantom lancer", "anti-mage", "juggernaut", "sven", "spectre",
        "faceless void", "morphling", "ursa", "terrorblade", "slark",
        "luna", "medusa", "gyrocopter", "chaos knight", "lifestealer",
        "drow ranger", "troll warlord", "monkey king", "weaver", "muerta",
        "phantom assassin", "bloodseeker", "clinkz", "arc warden",
    },
    "pos2": {
        "storm spirit", "ember spirit", "void spirit", "earth spirit",
        "puck", "lina", "leshrac", "invoker", "tinker", "queen of pain",
        "shadow fiend", "templar assassin", "dragon knight", "sniper",
        "zeus", "death prophet", "pangolier", "batrider", "meepo",
        "kunkka", "huskar", "viper", "necrophos", "outworld devourer",
        "tiny", "primal beast", "windranger", "pudge", "razor",
        "nature's prophet", "arc warden", "alchemist", "broodmother",
        "visage", "silencer",
    },
    "pos3": {
        "axe", "centaur warrunner", "mars", "tidehunter", "bristleback",
        "slardar", "underlord", "doom", "timbersaw", "magnus",
        "night stalker", "dawnbreaker", "beastmaster", "enigma",
        "dark seer", "legion commander", "viper", "primal beast",
        "abaddon", "brewmaster", "sand king", "lycan",
    },
    "pos4": {
        "tusk", "hoodwink", "clockwerk", "mirana", "bounty hunter",
        "nyx assassin", "earthshaker", "rubick", "tiny", "pugna",
        "techies", "dark willow", "snapfire", "skywrath mage", "marci",
    },
    "pos5": {
        "jakiro", "bane", "ancient apparition", "lion", "shadow shaman",
        "crystal maiden", "witch doctor", "ogre magi", "disruptor",
        "oracle", "grimstroke", "dazzle", "treant protector", "chen",
        "enchantress", "keeper of the light", "undying", "lich",
    },
}
...
def fetch_dota_news():
    global NEWS_CACHE
    if time.time() - NEWS_CACHE["timestamp"] < 600 and NEWS_CACHE["data"]:
        return NEWS_CACHE["data"]

    url = "https://api.steampowered.com/ISteamNews/GetNewsForApp/v0002/?appid=570&count=6&maxlength=300&format=json"
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=10, context=SSL_CONTEXT) as resp:
            raw = json.loads(resp.read().decode("utf-8"))
            items = raw.get("appnews", {}).get("newsitems", [])
            parsed = []
            for item in items:
                dt = datetime.datetime.fromtimestamp(
                    item.get("date", 0), tz=datetime.timezone.utc
                )
        ...
        print(f"Error fetching news from Steam: {e}")
...
async def analyze_draft(data: DraftRequest):
    ...
    for role_key, role_title in empty_positions.items():
        ...
        if not user_pick or user_pick.strip() == "":
            ...
            results.append({"role": role_title, "data": role_data})
